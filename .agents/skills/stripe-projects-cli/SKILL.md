...
Run `stripe projects switch-account` to choose an account or continue setup for this account. If a command exits with `INVALID_API_KEY`, the STRIPE_API_KEY environment variable is set but could not authenticate — unset it or provide a valid key, then retry. Do NOT retry the original command until the blocker is resolved.
...
| `INVALID_API_KEY` | STRIPE_API_KEY is set but failed to authenticate | Unset STRIPE_API_KEY or replace it with a valid key, then retry |
