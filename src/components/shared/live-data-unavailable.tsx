 ...
 */
export function LiveDataUnavailable({ title = "Live market data unavailable" }: { title?: string }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border/70 px-6 py-20 text-center">
      ...
      <p className="mt-1.5 max-w-md text-sm text-muted-foreground">
        Odds Hunter isn&apos;t connected to a live odds provider yet. Once Betfair (or another supported source) is
        configured, real fixtures and market data will appear here automatically.
      </p>
    </div>
  );
}
